...
public class ReportsController {
    ...
    public String getReports(ModelMap model, HttpServletRequest httpRequest) {
        ...
        if (result == null) {
            ...
            Report[] data;
            try {
                data = getReportsFromPowerBI(workspaceCollection, workspaceId, accessKey, apiUrl);
    ...
    private Report[] getReportsFromPowerBI(String workspaceCollection, String workspaceId, String accessKey, String apiUrl)
            throws Exception {
        URL url = new URL(String.format("%s/v1.0/collections/%s/workspaces/%s/reports",
                apiUrl, workspaceCollection, workspaceId));

        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestProperty("Authorization", String.format("AppKey %s", accessKey));
